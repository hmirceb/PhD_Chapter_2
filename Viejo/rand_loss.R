...
eliminar_end<-data_all %>%
  filter(LIPA_Endemic_pval <= 0.05 & Endemic == '1') %>%
...
q<-data.frame('Value' = c(unlist(rand_loss_end), 
  ...
  mutate(Rarity = factor(Rarity, levels = c('Endemic',
                                            'Regional geographic range',
                                            'Habitat specialization',
                                            'Local abundance'))) %>%
  group_by(Rarity) %>%
...
denstiy_plot<-ggplot(datos, aes(x = Value, fill = Rarity))+
  geom_density(alpha = 0.8, size = 0.7)+
  xlab('Proportion of PD lost')+
  ylab('Frequency')+
  geom_segment(data = data.frame(
    'Line' = c(obs_loss_end, obs_loss_ra, obs_loss_hs, obs_loss_la),
    'Rarity' = factor(c('Endemic',
                        'Regional geographic range',
                        'Habitat specialization',
                        'Local abundance'))),
    size = 1, 
    linetype = 'dashed',
    color = 'black', aes(x = Line, xend = Line, y = 0, yend = Inf))+
  geom_segment(data = q, linetype = 'dashed', aes(x = q, xend = q, yend = Inf, y = 0))+
  theme_bw()+
  theme(text = element_text(size = 20))+
  scale_fill_manual(values = c('Endemic' = '#FF8C00BF', 
                               'Regional geographic range' = 'firebrick',
                               'Habitat specialization' = 'steelblue',
                               'Local abundance' = 'forestgreen'), 
                    name = "Rarity type:", guide = 'none')+
  facet_wrap(~Rarity, nrow = 2, scales = 'free')+
  ggtitle('A')
  

mod<-lm(Value~Rarity, data = data.frame('Value' = c(unlist(rand_loss_end), 
                          unlist(rand_loss_ra),
                          unlist(rand_loss_hs),
                          unlist(rand_loss_la)),
              'Rarity' = rep(c('Endemic', 'RGR', 'HS', 'LA'), each = 999)))
anova(mod)
summary(mod)

ridge_plot<-ggplot(datos, aes(x = Value, fill = Rarity))+
  geom_density_ridges(aes(y = Rarity), alpha = 0.8, size = 0.7)+
  xlab('Proportion of PD lost')+
  ylab('Rarity type')+
  geom_segment(data = data.frame(
    'Line' = c(obs_loss_end, obs_loss_ra, obs_loss_hs, obs_loss_la),
    ...
    size = 1, 
    linetype = 'dashed', aes(color = Rarity, x = Line, xend = Line, y = 1:4, yend = 2:5))+
  theme_bw()+
  theme(text = element_text(size = 20))+
  scale_fill_manual(values = c('Endemic' = '#FF8C00BF', 
                               'Regional geographic range' = 'firebrick',
                               ...
                               'Local abundance' = 'forestgreen'), 
                    name = "Rarity type:", guide = 'none')+
  scale_color_manual(values = c('Endemic' = '#FF8C00BF', 
                                'RGR' = 'firebrick',
                                'HS' = 'steelblue',
                                'LA' = 'forestgreen'), 
                     name = "Rarity type:", guide = 'none')
...
ggarrange(denstiy_plot, ses_loss_plot, ncol = 1, common.legend = T,
          legend = 'bottom')
...
data_all<- data_all %>%
  ...
  mutate(end_sig = ifelse(LIPA_Endemic_pval <= 0.05 & Endemic == 1, 'Rare', 'Common'),
         ra_sig = ifelse(LIPA_RA_max_pval <= 0.05 & scale(RA_max) < 0, 'Rare', 'Common'),
         hs_sig = ifelse(LIPA_B_h_pval <= 0.05 & scale(B_h) < 0, 'Rare', 'Common'),
         la_sig = ifelse(LIPA_LA_pval <= 0.05 & scale(LA) < 0, 'Rare', 'Common'))
...
save(m ,m1, m2, m3, file = 'C:/Users/18172844S/Dropbox/DATA__LAB/Hector_tesis/Cap. 2 - Senal Filogenetica en la rareza del Pirineo/Resultados/Modelos_taxo.RData')
